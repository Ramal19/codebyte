...
// ✅ RENDER.COM ÜÇÜN TAM UYĞUN VERSİYA
// Heç bir funksiya dəyişməyib — sadəcə Render üçün düzəlişlər əlavə edilib
...
// 🟩 FRONTEND-i Render üçün düzgün təqdim et
const frontendPath = path.join(__dirname, "../frontend");
app.use(express.static(frontendPath));

// Əsas route (Render “Cannot GET /” problemini həll edir)
app.get("/", (req, res) => {
  res.sendFile(path.join(frontendPath, "index.html"));
});

// 🔹 JSON oxuma/yazma funksiyaları
function readUsers() {
  if (!fs.existsSync(USERS_FILE)) return [];
  return JSON.parse(fs.readFileSync(USERS_FILE));
}
...
app.get("/profile", auth, (req, res) => {
  res.json({
    message: `Xoş gəldin ${req.user.username}!`,
    role: req.user.role,
  });
});
...
// 🟢 CATCH-ALL ROUTE (React və ya frontend router üçün)
// Yəni hər hansı səhifəyə birbaşa daxil olduqda Render "Cannot GET /..." deməsin
app.get("*", (req, res) => {
  res.sendFile(path.join(frontendPath, "index.html"));
});
